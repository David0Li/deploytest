...
function topicDOMObject(commentJSON) {
  commentDiv = document.createElement('div');
  commentDiv.setAttribute('id', commentJSON._id);
  commentDiv.className = 'topic mb-2';
...
function newCommentDOMObject(parent) {
  ...
  newCommentContent.setAttribute('placeholder', 'Comment');
...
function newStoryDOMObject(placeholder, enter_key) {
  ...
  newStoryContent.setAttribute('placeholder', placeholder);
  ...
  newStorySubmit.innerHTML = enter_key;
...
function renderStories(user) {
  if (user._id !== undefined)
    document.getElementById('new-story').appendChild(newStoryDOMObject("Ask a Question", "Submit"));
  //const topicDiv = document.getElementById('topics');
  const storiesDiv = document.getElementById('stories');
  //topicDiv.prepend(topicDOMObject(story, user));
